"""ExtraTreesClassifier (max_features=0.3) + another_data OOF + test parquets.

Pilot: max_features=0.3 action argmax F1=0.31278 vs sqrt 0.29766 (+0.01512).
More features per split (30% = ~90 vs sqrt = ~17) gives better signal while
keeping random threshold diversity. Different from extratrees_extra (sqrt).

CPU-only. Writes:
  artifacts/oof/extratrees_03_extra_{action,point,server}.parquet (+test)
"""
...
MODEL_NAME = "extratrees_03_extra"
N_ESTIMATORS = 300
MAX_FEATURES = 0.3
MIN_SAMPLES_LEAF = 5
...
def main():
    ...
    print(f"{MODEL_NAME}: ET n={N_ESTIMATORS} max_features={MAX_FEATURES}", flush=True)
    ...
    for seed, fold, train_view, valid_view in iter_cv_folds(train, splits):
        ...
        for tgt, classes, y_col, s_off in [
            ("action", TARGET_ACTION_CLASSES, "y_actionId", 0),
            ("point", TARGET_POINT_CLASSES, "y_pointId", 1000),
        ]:
            clf = ExtraTreesClassifier(n_estimators=N_ESTIMATORS, max_features=MAX_FEATURES,
                min_samples_leaf=MIN_SAMPLES_LEAF, random_state=seed+fold*100+s_off,
        ...
        clf_s = ExtraTreesClassifier(n_estimators=N_ESTIMATORS, max_features=MAX_FEATURES,
    ...
    for tgt, classes, y_col, s_off in [
        ("action", TARGET_ACTION_CLASSES, "y_actionId", 0),
        ("point", TARGET_POINT_CLASSES, "y_pointId", 1),
    ]:
        clf = ExtraTreesClassifier(n_estimators=N_ESTIMATORS, max_features=MAX_FEATURES,
            min_samples_leaf=MIN_SAMPLES_LEAF, random_state=99999+s_off, n_jobs=-1,
