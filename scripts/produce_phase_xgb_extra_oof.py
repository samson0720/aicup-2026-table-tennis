"""Phase-specific XGBoost + another_data augmentation OOF + test parquets (GPU).

Combines phase splitting (train_phase_lgbm) with XGBoost GPU training and
another_data augmentation. Writes:
  artifacts/oof/phase_xgb_extra_{action,point,server}.parquet  (+test)

Usage:
  CUDA_VISIBLE_DEVICES=1 conda run -n aicup-tt python -m scripts.produce_phase_xgb_extra_oof
"""
...
MODEL_NAME = "phase_xgb_extra"
N_ESTIMATORS = 600
DEVICE = "cuda"
MIN_PHASE_ROWS = 100


def _phase_predict_xgb(df_train: pd.DataFrame, df_valid: pd.DataFrame, feats: list[str]) -> tuple:
    ...
    for phase in sorted(set(int(p) for p in valid_phase)):
        ...
        p_action[val_mask] = fit_mc(xt, ya_t, xv, len(TARGET_ACTION_CLASSES),
                                    2026 + int(phase) * 10, N_ESTIMATORS, DEVICE)
        p_point[val_mask] = fit_mc(xt, yp_t, xv, len(TARGET_POINT_CLASSES),
                                   3026 + int(phase) * 10, N_ESTIMATORS, DEVICE)
        p_server[val_mask] = fit_bin(xt, ys_t, xv, 4026 + int(phase) * 10, N_ESTIMATORS, DEVICE)
...
def main() -> None:
    ...
    print(f"{MODEL_NAME}: device={DEVICE} n_est={N_ESTIMATORS}", flush=True)
    ...
    for seed, fold, train_view, valid_view in iter_cv_folds(train, splits):
        ...
        pa, pp, ps = _phase_predict_xgb(df_train, df_valid, feats)
    ...
    for phase in sorted(set(int(p) for p in test_phase)):
        ...
        p_action[test_mask] = fit_mc(xt, ya, xv, len(TARGET_ACTION_CLASSES),
                                     8200 + int(phase), N_ESTIMATORS, DEVICE)
        p_point[test_mask] = fit_mc(xt, yp, xv, len(TARGET_POINT_CLASSES),
                                    8300 + int(phase), N_ESTIMATORS, DEVICE)
        p_server[test_mask] = fit_bin(xt, ys, xv, 8400 + int(phase), N_ESTIMATORS, DEVICE)
