"""Phase-specific XGBoost (max_depth=8) + another_data OOF + test parquets (GPU).

Same as phase_xgb_extra but max_depth=8 (vs 6). Deeper trees per phase may
capture more complex interactions with smaller per-phase training sets.
Writes:
  artifacts/oof/phase_xgb8_extra_{action,point,server}.parquet  (+test)

Usage:
  CUDA_VISIBLE_DEVICES=1 conda run -n aicup-tt python -m scripts.produce_phase_xgb8_extra_oof
"""
...
def fit_mc(x_tr, y_tr, x_va, n_cls, seed, n_est, device, depth):
    le = LabelEncoder().fit(y_tr)
    clf = xgb.XGBClassifier(objective="multi:softprob", tree_method="hist",
                            device=device, n_estimators=n_est, max_depth=depth,
...
def fit_bin(x_tr, y_tr, x_va, seed, n_est, device, depth):
...
def _phase_predict(df_train, df_valid, feats):
    ...
    for phase in sorted(set(int(p) for p in valid_phase)):
        ...
        p_action[val_mask] = fit_mc(xt, ya_t, xv, len(TARGET_ACTION_CLASSES),
                                    2026 + int(phase) * 10, N_ESTIMATORS, DEVICE, MAX_DEPTH)
        p_point[val_mask] = fit_mc(xt, yp_t, xv, len(TARGET_POINT_CLASSES),
                                   3026 + int(phase) * 10, N_ESTIMATORS, DEVICE, MAX_DEPTH)
        p_server[val_mask] = fit_bin(xt, ys_t, xv, 4026 + int(phase) * 10, N_ESTIMATORS, DEVICE, MAX_DEPTH)
...
def main() -> None:
    ...
    for phase in sorted(set(int(p) for p in test_phase)):
        ...
        p_action[test_mask] = fit_mc(xt, ya, xv, len(TARGET_ACTION_CLASSES),
                                     8200 + int(phase), N_ESTIMATORS, DEVICE, MAX_DEPTH)
        p_point[test_mask] = fit_mc(xt, yp, xv, len(TARGET_POINT_CLASSES),
                                    8300 + int(phase), N_ESTIMATORS, DEVICE, MAX_DEPTH)
        p_server[test_mask] = fit_bin(xt, ys, xv, 8400 + int(phase), N_ESTIMATORS, DEVICE, MAX_DEPTH)
