"""Phase-specific CatBoost (depth=10, 800 iter) + another_data OOF + test parquets (GPU).

phase_cat8_800_extra (depth=8, 800 iter) was SHIPPED +0.00181. Tests whether depth=10
provides further lift (same as XGBoost depth ladder: 8→10 gave +0.00156 for XGB).

Usage:
  CUDA_VISIBLE_DEVICES=1 conda run -n aicup-tt python -m scripts.produce_phase_cat10_extra_oof
"""
...
def _phase_predict_cat(df_train, df_valid, feats, cat_idx, cat_cols):
    ...
    for phase in sorted(set(int(p) for p in valid_phase)):
        ...
        p_action[val_mask] = fit_multiclass(xt, df_train.loc[trn_mask,"y_actionId"], xv,
            TARGET_ACTION_CLASSES, cat_idx, "sqrt", 9500+int(phase)*10, ITERATIONS, DEPTH, TASK_TYPE, DEVICES)
        p_point[val_mask] = fit_multiclass(xt, df_train.loc[trn_mask,"y_pointId"], xv,
            TARGET_POINT_CLASSES, cat_idx, "sqrt", 9600+int(phase)*10, ITERATIONS, DEPTH, TASK_TYPE, DEVICES)
        p_server[val_mask] = fit_binary(xt, df_train.loc[trn_mask,"y_serverGetPoint"], xv,
            cat_idx, 9700+int(phase)*10, ITERATIONS, DEPTH, TASK_TYPE, DEVICES)
...
def main():
    ...
    print(f"{MODEL_NAME}: DEPTH={DEPTH} ITER={ITERATIONS} task_type={TASK_TYPE}", flush=True)
    ...
    for phase in sorted(set(int(p) for p in test_phase)):
        ...
        am = fit_full_multiclass(xt, ya, TARGET_ACTION_CLASSES, cat_idx, "sqrt", 8500+int(phase), ITERATIONS, DEPTH, TASK_TYPE, DEVICES)
        pm = fit_full_multiclass(xt, yp, TARGET_POINT_CLASSES, cat_idx, "sqrt", 8600+int(phase), ITERATIONS, DEPTH, TASK_TYPE, DEVICES)
        sm = fit_full_binary(xt, ys, cat_idx, 8700+int(phase), ITERATIONS, DEPTH, TASK_TYPE, DEVICES)
