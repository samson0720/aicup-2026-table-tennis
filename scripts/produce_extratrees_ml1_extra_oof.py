"""ExtraTreesClassifier (min_samples_leaf=1) + another_data OOF + test parquets.

Pilot: ET_ml1 action argmax F1=0.31760 vs ET_300 (min_sl=5) 0.29766 (+0.01994).
Fully grown trees (min_samples_leaf=1) capture rare action class patterns more precisely.

CPU-only. Writes:
  artifacts/oof/extratrees_ml1_extra_{action,point,server}.parquet (+test)
"""
...
MODEL_NAME = "extratrees_ml1_extra"
N_ESTIMATORS = 300
MIN_SAMPLES_LEAF = 1
...
def main():
    ...
    print(f"{MODEL_NAME}: ET n={N_ESTIMATORS} min_sl={MIN_SAMPLES_LEAF}", flush=True)
    ...
    for seed, fold, train_view, valid_view in iter_cv_folds(train, splits):
        ...
        clf_a = ExtraTreesClassifier(n_estimators=N_ESTIMATORS, max_features="sqrt",
            min_samples_leaf=MIN_SAMPLES_LEAF, random_state=seed+fold*100, n_jobs=-1,
        ...
        for i, c in enumerate(clf_a.classes_): pa[:, int(c)] = raw_a[:, i]

        clf_p = ExtraTreesClassifier(n_estimators=N_ESTIMATORS, max_features="sqrt",
            min_samples_leaf=MIN_SAMPLES_LEAF, random_state=seed+fold*100+1000, n_jobs=-1,
            class_weight="balanced_subsample")
        ...
        for i, c in enumerate(clf_p.classes_): pp[:, int(c)] = raw_p[:, i]
    ...
    for tgt, classes, y_col in [("action", TARGET_ACTION_CLASSES, "y_actionId"),
        ...
        clf = ExtraTreesClassifier(n_estimators=N_ESTIMATORS, max_features="sqrt",
            min_samples_leaf=MIN_SAMPLES_LEAF, random_state=99999, n_jobs=-1,
            class_weight="balanced_subsample")
