"""Phase-specific LGBM 300 iterations + another_data OOF + test parquets.

Same as phase_lgbm_extra but with 300 estimators (vs 180). Per-phase models
with smaller training sets may benefit more from additional trees.
Writes:
  artifacts/oof/phase_lgbm_extra_300_{action,point,server}.parquet  (+test)

Usage:
  conda run -n aicup-tt python -m scripts.produce_phase_lgbm_extra_300_oof
"""
...
MODEL_NAME = "phase_lgbm_extra_300"
N_ESTIMATORS = 300
NUM_LEAVES = 31
MIN_PHASE_ROWS = 100
...
def _phase_predict(df_train, df_valid, feats):
    ...
    for phase in sorted(set(int(p) for p in valid_phase)):
        ...
        xt, xv = x_train.iloc[trn_mask], x_valid.iloc[val_mask]
        p_action[val_mask] = fit_multiclass(xt, df_train.loc[trn_mask, "y_actionId"], xv,
            df_valid.loc[val_mask, "y_actionId"], TARGET_ACTION_CLASSES, "sqrt",
            8200 + int(phase) * 10, N_ESTIMATORS, NUM_LEAVES)
        p_point[val_mask] = fit_multiclass(xt, df_train.loc[trn_mask, "y_pointId"], xv,
            df_valid.loc[val_mask, "y_pointId"], TARGET_POINT_CLASSES, "sqrt",
            8300 + int(phase) * 10, N_ESTIMATORS, NUM_LEAVES)
        p_server[val_mask] = fit_binary(xt, df_train.loc[trn_mask, "y_serverGetPoint"], xv,
            df_valid.loc[val_mask, "y_serverGetPoint"], 8400 + int(phase) * 10, N_ESTIMATORS, NUM_LEAVES)
...
def main():
    ...
    for phase in sorted(set(int(p) for p in test_phase)):
        ...
        xt, xv = x_train_full.iloc[train_mask], x_test.iloc[test_mask]
        ya, yp = full_df_train.loc[train_mask, "y_actionId"], full_df_train.loc[train_mask, "y_pointId"]
        ys = full_df_train.loc[train_mask, "y_serverGetPoint"]
        am = fit_multiclass_full(xt, ya, TARGET_ACTION_CLASSES, "sqrt", 8200 + int(phase), N_ESTIMATORS, NUM_LEAVES)
        pm = fit_multiclass_full(xt, yp, TARGET_POINT_CLASSES, "sqrt", 8300 + int(phase), N_ESTIMATORS, NUM_LEAVES)
        sm = fit_binary_full(xt, ys, 8400 + int(phase), N_ESTIMATORS, NUM_LEAVES)
