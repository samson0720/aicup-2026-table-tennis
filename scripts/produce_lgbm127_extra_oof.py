"""lgbm127_extra — LGBM with 127 leaves + another_data augmentation.

Higher capacity than lgbm63_extra (63 leaves). Writes:
  artifacts/oof/lgbm127_extra_{action,point,server}.parquet  (+test)

Usage:
  conda run -n aicup-tt python -m scripts.produce_lgbm127_extra_oof
"""
...
from scripts.train_lgbm_baseline import (
    TARGET_ACTION_CLASSES, TARGET_POINT_CLASSES, feature_columns, fit_binary, fit_multiclass,
)

MODEL_NAME = "lgbm127_extra"
NUM_LEAVES = 127
N_ESTIMATORS = 180
...
def main():
    ...
    ma = fit_multiclass_full(x_full, full_df_train["y_actionId"], TARGET_ACTION_CLASSES, "sqrt", 2026, N_ESTIMATORS, NUM_LEAVES)
    mp = fit_multiclass_full(x_full, full_df_train["y_pointId"], TARGET_POINT_CLASSES, "sqrt", 3026, N_ESTIMATORS, NUM_LEAVES)
