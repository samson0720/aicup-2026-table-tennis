"""Phase5-specific XGBoost (max_depth=8) + another_data OOF + test parquets (GPU).

Combines 5 fine game-phase buckets with deeper trees (max_depth=8 vs 6).
Phase buckets: 0=serve(2), 1=receive(3), 2=third-ball(4), 3=early(5-6), 4=extended(>=7)
Writes:
  artifacts/oof/phase5_xgb8_extra_{action,point,server}.parquet  (+test)

Usage:
  CUDA_VISIBLE_DEVICES=1 conda run -n aicup-tt python -m scripts.produce_phase5_xgb8_extra_oof
"""
...
def _phase_predict(df_train, df_valid, feats):
    ...
    for phase in sorted(set(int(p) for p in valid_phase)):
        ...
        p_action[val_mask] = fit_mc(xt, ya_t, xv, len(TARGET_ACTION_CLASSES), 2026 + phase * 10)
        p_point[val_mask] = fit_mc(xt, yp_t, xv, len(TARGET_POINT_CLASSES), 3026 + phase * 10)
        p_server[val_mask] = fit_bin(xt, ys_t, xv, 4026 + phase * 10)
...
def main():
    ...
    for phase in sorted(set(int(p) for p in test_phase)):
        ...
        xv = x_test[test_mask]
        p_action[test_mask] = fit_mc(xt, ya, xv, len(TARGET_ACTION_CLASSES), 8200 + phase)
        p_point[test_mask] = fit_mc(xt, yp, xv, len(TARGET_POINT_CLASSES), 8300 + phase)
        p_server[test_mask] = fit_bin(xt, ys, xv, 8400 + phase)
