"""Phase-specific XGBoost (max_depth=8, 900 iterations) + another_data OOF + test (GPU).

Same as phase_xgb8_extra but with 900 estimators (vs 600). Tests whether more trees
at depth=8 improve over the shipped phase_xgb8_extra.
Writes: artifacts/oof/phase_xgb8_900_extra_{action,point,server}.parquet  (+test)

Usage:
  CUDA_VISIBLE_DEVICES=0 conda run -n aicup-tt python -m scripts.produce_phase_xgb8_900_extra_oof
"""
...
MODEL_NAME = "phase_xgb8_lr03_extra"
N_ESTIMATORS = 1200
MAX_DEPTH = 8
DEVICE = "cuda"
MIN_PHASE_ROWS = 100
