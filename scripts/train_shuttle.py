"""ShuttleForecaster training + OOF + test-inference script.

Closely mirrors scripts/train_seq_pilot.py but uses ShuttleForecaster
(action + point heads only, no server head/loss).
Loss = CE(action) + CE(point), each class-weighted by inverse-sqrt class frequency.
"""
...
def _class_weights(labels: np.ndarray, n_classes: int, device: torch.device) -> torch.Tensor:
    """Inverse-sqrt class frequency weights, normalized to mean=1, guarded vs. zero counts."""
    counts = np.bincount(labels.astype(int), minlength=n_classes).astype(float)
    counts = counts + 1.0  # guard zero counts
...
def _predict(model: nn.Module, loader: DataLoader, device: torch.device) -> dict[str, np.ndarray]:
    ...
    return {key: np.concatenate(values, axis=0) for key, values in cols.items()}
...
def _monitor_score(
    ...
    """Cheap early-stop monitor: argmax macro-F1 for action and point."""
    from sklearn.metrics import f1_score

    action_pred = action_probs.argmax(1)
    point_pred = point_probs.argmax(1)
    action_f1 = float(f1_score(y_action, action_pred, average="macro", zero_division=0))
    point_f1 = float(f1_score(y_point, point_pred, average="macro", zero_division=0))
...
def run_one_fold(
    ...
    best = {"overall": -1.0, "action_f1": 0.0, "point_f1": 0.0}
    best_state = None
    bad_epochs = 0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        monitor = _monitor_score(val["action"], val["point"], val["y_action"], val["y_point"])
        print(
            f"seed{seed} fold{fold} epoch{epoch} "
            f"train_loss={last_train_loss:.4f} "
            f"val_overall={monitor['overall']:.4f} "
...
def _write_test_parquet(pred: dict[str, np.ndarray], model_name: str) -> None:
...
def run_predict_test(
    ...
    and write single-cut test predictions.
    """
    seed = args.seeds[0]
...
def main() -> None:
    ...
    parser.add_argument("--model-name", default="shuttle")
